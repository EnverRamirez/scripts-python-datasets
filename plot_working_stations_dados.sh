...
stations=$(cut -d';' -f2 ../dados/${x}.txt | uniq | grep -v Estacao| wc -l) #get a unique name then count
echo $x $stations
done > different_Stations_dados.txt

year_with_more_stations=$(sort -k2,2nr different_Stations_dados.txt | head -n1 | awk '{print $1}')
number_of_stations=$(sort -k2,2nr different_Stations_dados.txt | head -n1 | awk '{print $2}') #sorting by column in numerically reverse (descending) order

echo $year_with_more_stations $number_of_stations

#Get the station list of the year with more stations
station_list=$(cut -d';' -f2 ../dados/${year_with_more_stations}.txt | uniq | grep -v Estacao)

#now plot the data using also python3
for station in ${station_list[@]}
do
for x in $(seq $start_year $end_year);do more ../dados/${x}.txt |grep $station; done > tmp.txt 
...
#plt.tight_layout()
#plt.show()
plt.savefig("../plot_dados/${station}.png")
plt.close()
EOF
###########################################
